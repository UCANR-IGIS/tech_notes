...
The current version of RTools is 4.5, however looking at the [RTools compatibility table](https://cran.r-project.org/bin/windows/Rtools/history.html) we see that R 3.6.3 is only compatible with  **RTools 3.5** and earlier. Hence we should download and install RTools 3.5. 

::: {.callout-tip title="Tips for installing RTools"}
1. When installing RTools, accept the default location (e.g., C:\\Rtools on Windows). If you put it somewhere else, things can break.
...
::: {.callout-tip title="Alternative: Posit Package Manager"}

An alternative to the `groundhog` package is to use historical snapshots of CRAN from the Posit Package Manager. By setting the repository URL to a specific date, R treats CRAN exactly as it was on that day. 

```{r eval = FALSE}
options(repos = c(CRAN = "https://packagemanager.posit.co/cran/2020-01-01")))
```

After you've 'redirected' the CRAN repo URL to a date-stamped version of the Posit Package Manager, you can install packages as usual with `install.packages()`, and it will be like you are back in January 2020. 

A bonus of using the Posit Package Manager (assuming it works) is that it provides pre-compiled binaries on some operating systems. This might allow you to skip installing RTools.
:::
...
The function to install old packages is `groundhog.library()`, which can be run in a loop:
...
To check if `tlocoh` is fully working, you can work thru [this script](https://github.com/UCANR-IGIS/tech_notes/blob/main/docs/tlocoh-tutorial-code.R) which follows the  [tutorial  Vignette](https://tlocoh.r-forge.r-project.org/tlocoh_tutorial_2014-08-17.pdf). Remember though that you must be working in R 3.6.3, because that's the version that has `tlocoh` installed.
...
Because you can't run `tlocoh` in the latest version of R, your best option is to export outputs to disk in your time machine, switch to the current release of R, and import them.

You can save `tlocoh's` outputs to disk in standard formats compatible with any version of R. For `tlocoh's` spatial outputs, this could include standard GIS formats like Shapefiles, which you can create using functions from `tlocoh` or `rgdal`. Even better, you can export them as native R objects (e.g., rds files created with `saveRDS()`), and then import them as data frames and `sp` objects. This will take a little digging into `tlocoh's` data formats (but see the Vignette on [T-LoCoH Data Classes](https://github.com/r-forge/tlocoh/blob/master/pkg/inst/doc/tlocoh_data_classes.pdf)).
