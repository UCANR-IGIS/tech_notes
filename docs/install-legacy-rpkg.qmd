...
[`tlocoh`](https://tlocoh.r-forge.r-project.org/) is a brilliant package to analyze spatiotemporal patterns in animal movement. Unfortunately, the last real update to tlocoh was in 2013, and since then several of its dependent packages have been retired, including `rgdal`, `rgeos`, and `gcplib`. To make matters worse, some of the dependencies are no longer compatible with the most recent version R, meaning we'll have to install an older version of R as well.
