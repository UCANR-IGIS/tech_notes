...
```{css echo = FALSE}
h2,h1.title {font-weight:bold;}
h4.date {font-size:90%;}
#anr_header {background-color:#1c2c53; color:white; padding:20px;}
```
...
Now we can import the park boundary which is saved as a [geojson file](placertia_canyon_sp.geojson). We use the [`sf`](https://r-spatial.github.io/sf/){target="_blank" rel="noopener"} package to import and transform the boundary to geographic coordinates, and [`tmap`](https://r-tmap.github.io/tmap/){target="_blank" rel="noopener"} to plot it:
...
Next, in order to spatially filter the observations using the park boundary, we convert the data frame into a sf object. At the same time we'll select just a few of the rows:
...
$(document).ready(function() {
  $('.main-container').prepend('<div id=\"anr_header\"><img src=\"images/ucanr_2019_white_232x30.png\" style=\"width:232px; height:30px; margin-bottom:10px;\"/><a href=\"http://igis.ucanr.edu/\"><img src=\"images/igis-shield_bg-white_40x40.png\" style=\"width:40px; height:40px; float:right;\"/></a></div><div style="clear:both;"></div>');
});
```
